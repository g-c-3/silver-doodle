...
**Ad-life grants and bonus-round entries (Phase 10, fully deployed and verified live 2026-09-19).** Neither `adLifeUsed` nor `isBonus` in the payload above is trusted as a bare client-reported boolean. Every rewarded-ad request `client/src/js/attempt.js`'s `playRewardedAd()` makes sets `ssv.customData = ${attemptId}:${slotIndex}:life|bonus` and `ssv.userId` to the player's own id; Google's AdMob infrastructure calls `server/functions/admob-ssv/index.ts` directly and out-of-band from the device once the ad genuinely completes, carrying those same two values in a request whose query parameters are cryptographically signed by Google. `admob-ssv` verifies that signature (ECDSA P-256/SHA-256, DER-decoded to raw before verifying — see below — against Google's published, rotating key set) before writing a row to `ad_verifications` (`attempt_id, slot_index, ad_type, transaction_id`, service-role-only — no client RLS access in either direction). `score-replay` requires a matching row here before crediting any level with `adLifeUsed=true` or `isBonus=true` — a modified client can still set either flag to whatever it wants in its own payload, but without an independently-verified ad completion recorded against that exact attempt+slot+type, the level is rejected outright, same as an illegal move or a tampered board. "No ad available" (AdMob no-fill) does **not** bypass this — the client shows honest wording for that specific case but still declines the same as any other ad failure; granting it for free would reopen this exact hole (see docs/DECISIONS.md's 2026-09-19 entry).

Confirmed live end to end: migration run, `admob-ssv` and `score-replay` both deployed and reachable, SSV callback URL configured in the AdMob console, signature verification confirmed against a real live "Verify callback URL" test (after fixing a real bug — see below), and a full on-device rewarded-ad watch-through completed for the ad-life grant (an initial "Publisher data not found" error was a known, time-based AdMob onboarding state for a new ad unit, not a bug — resolved after about an hour, per Google's own error documentation).

Migration run and `admob-ssv` deployed (2026-09-18). AdMob console's "Verify callback URL" flow (there's no separately-labeled "Send test callback" button in the current UI — that flow *is* the real signed-callback test) surfaced a real bug the same day: the verification code assumed Google's `signature` parameter was already a raw, fixed-width r‖s ECDSA signature, but Google's own reference implementation confirms it's DER-encoded (ASN.1 SEQUENCE of two INTEGERs) — WebCrypto's `verify()` silently returns `false` rather than throwing when handed the wrong format, which is exactly what the logs showed. Fixed with a `derSignatureToRaw()` conversion step, reproduced against the exact failure and confirmed against a full simulated callback locally before shipping, then **re-confirmed against a second live "Verify url" test the same day — passed** (log line `"admob-ssv: malformed custom_data \"null\"..."` means the signature check succeeded; it only stopped on the test callback's deliberately-blank custom_data). Full on-device rewarded-ad watch-through completed 2026-09-19.

Still not yet done: set the AdMob console's rewarded ad unit's real production Callback URL if not already saved (it was, per the same-day screenshots) and re-run "Verify url" to confirm the fix against a live signed callback, then a full on-device rewarded-ad watch-through for both the ad-life grant and bonus-round entry.
