...
Migration run and `admob-ssv` deployed (2026-09-18) — confirmed live via Supabase's own function tester (correctly 405s a POST, since AdMob only ever sends GET). AdMob console's "Verify callback URL" flow (there's no separately-labeled "Send test callback" button in the current UI — that flow *is* the real signed-callback test) surfaced a real bug the same day: the verification code assumed Google's `signature` parameter was already a raw, fixed-width r‖s ECDSA signature, but Google's own reference implementation confirms it's DER-encoded (ASN.1 SEQUENCE of two INTEGERs) — WebCrypto's `verify()` silently returns `false` rather than throwing when handed the wrong format, which is exactly what the logs showed. Fixed with a `derSignatureToRaw()` conversion step, reproduced against the exact failure and confirmed against a full simulated callback locally before shipping — **not yet re-confirmed against a second live "Verify url" test**, which is the immediate next step. See docs/SESSIONS.md's latest entry for the full writeup.
