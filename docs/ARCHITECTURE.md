...
- **Idle hints (added 2026-09-15):** after 5 seconds with no successful move (an invalid swap attempt doesn't count), every tile that's part of at least one currently-available match-producing swap is highlighted with a pulsing colour glow. The highlight persists — it does not re-flicker or re-scan — until the player's next successful move, which both clears it and restarts the 5-second countdown from zero. Applies to bonus levels too. See `GameEngine.findHintCells()` and `attempt.js`'s `showHints()`/`scheduleHintTimer()`/`clearHints()`.
