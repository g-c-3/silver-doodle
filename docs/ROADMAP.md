...
- [ ] **Phase 1 — Infra & secrets (manual, one-time).** Android signing keystore generated and stored as a repo secret. Supabase project created; access token and project ref stored as repo secrets. Firebase project created for Analytics + Crashlytics only. AdMob account and app created, test ad unit IDs obtained.
