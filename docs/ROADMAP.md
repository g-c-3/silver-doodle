...
- [ ] **Phase 2 — Database schema.** Migration file written and delivered (`supabase/migrations/20260912000000_phase2_schema.sql`) covering `users`, `daily_game_definitions` + `daily_game_definition_slots`, `player_daily_order`, `attempts`, `daily_stats`, `weekly_stats`, `all_time_stats`, `user_year_activity`, plus RLS locking all writes to service-role only. Not yet checked off — pending the migration actually being run against the live Supabase project via the SQL Editor and confirmed successful.
